...
#bx,by,ix,iy = 0,0,0,0 # global variable for image x,y and box x,y 
#global bx,by,ix,iy
class Drone(object):
######################################=-
...
##############################################################=-
	...
	'''@property
	def confbx(self): return self.__bx'''
######################=-
### Drone commands ###=-
######################=-
...
#	cv2.startWindowThread()
	...
	while not commitsuicideV:
		...
		if not largest_contour == None:
			...
			# end straight bounding rectangle--------------------------------'''
			if moment["m00"] > 1000 / scale_down:
				...
				'''self.bx = cbox[0]
				self.by = cbox[1]
				self.ix = len(image[0])/2
				self.iy = len(image)/2'''
				print "In ps-drone, the image is", image, '\n'
				#print "In ps_drone, The box's center is: " ,cbox,'\n'  #box center will print like [xx,xx]
				#print "In ps_drone, The image center is: ", len(image[0])/2, len(image)/2, '\n'	#image is many rows of same-sized array, therefore, len(image) is the number of rows and len(image[0]) is the number of columns.
				...
				if count > 0:
					...
					if show:
						...
						if cv2.waitKey(20) == 27:
							...
							self.capture.release()
							break
						'''# straight round rectangle-------------------
						if cv2.waitKey(20) == 27:
							cv2.destroyWindow("ColourTrackerWindow")
							self.capture.release()
							break	
						#end straight round rectangle-----------------'''					
