		...
		while self.true == True:
			if self.q.empty() is False:
				...
				if len(item2) != 0:
					...
					#self.mydrone.mtrim()
					if (bx-ix)<0 and (by-iy)<0:  #left upper , move up and turn right, tag = #1
						...
						self.oldx = bx
						self.oldy = by
					elif (bx-ix)<0 and (by-iy)>0: #left lower, move down and turn right, tag = #2
						...
						if self.tag == 1:
							...
							self.mydrone.stop()
						self.tag = 2
					elif (bx-ix)>0 and (by-iy)<0:  #right upper, move up and turn left, #3
						'''if self.tag!=1 and self.tag !=3:
							self.mydrone.moveUp()
						...
						if self.tag == 1:
							...
							self.mydrone.stop()
						self.tag = 3
					elif (bx-ix)>0 and (by-iy)>0: #right lower, move down and turn left, #4
						'''if self.tag!=2 and self.tag !=4:					
							self.mydrone.moveDown()
						if self.tag!=3 and self.tag !=4:
							self.mydrone.turnLeft()'''
