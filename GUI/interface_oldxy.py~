...
class Ar_drone:
	...
	def moveLeft(self):
		...
		self.drone.move(-0.05,0.0,0.0,0.0)
		print "a --- Move left"
	def moveRight(self):
	...
	def moveDown(self):
		...
		self.drone.move(0.0,0.0,-0.3,0.0)
		print "s --- Move Down"
	def turnLeft(self):
		...
		#self.drone.mtrim()
		self.drone.relMove(0.0,0.0,0.0,-0.15,0,0)	
	...
	def move(self,right,forward,up,turn):
		...
		while self.true == True:
			if self.q.empty() is False:
				...
				if len(item2) != 0:
					...
					#self.mydrone.mtrim()
					if (bx-self.oldx)<0 and (by-self.oldy)<0:  #left upper , move up and turn right, tag = #1
						self.mydrone.move(0.1,0,0.2,0)
						self.tag = 1				
					elif (bx-self.oldx)<0 and (by-self.oldy)>0: #left lower, move down and turn right, tag = #2
						self.mydrone.move(0.1,0,0.4,0)
						self.tag = 2
					elif (bx-self.oldx)>0 and (by-self.oldy)<0:  #right upper, move up and turn left, #3
						self.mydrone.move(-0.1,0,0.4,0)
						self.tag = 3
					elif (bx-self.oldx)>0 and (by-self.oldy)>0: #right lower, move down and turn left, #4
						self.mydrone.move(-0.1,0, -0.4,0)
						self.tag = 4
					self.oldx = bx
					self.oldy = by
